...
Combine different operation types — swaps, staking, lending (supply, borrow, repay, withdraw), sends, and DCA orders — in one atomic transaction:
...
Requires at least 2 operations. Each flag (`--swap`, `--stake`, `--supply`, `--send`, `--borrow`, `--repay`, `--withdraw`, `--dca-create`, `--dca-cancel`) can be repeated.
...
Supply, borrow, repay, monitor health, and auto-rebalance lending positions:
...
npx starkfi@latest lend-status -p Prime \                    # View specific position
  --collateral-token ETH
npx starkfi@latest lend-monitor                               # Health monitoring (risk levels)
npx starkfi@latest lend-auto -p Prime \                       # Auto-rebalance risky position
...
Set up and manage recurring buy orders:

```bash
npx starkfi@latest dca-preview 10 USDC ETH                                # Preview one cycle
...
npx starkfi@latest dca-cancel 1fd90ab7-34bf-42b8-afa0-011daf6f15ba         # Cancel using ID
npx starkfi@latest dca-cancel 0x123...                                     # Cancel using address
```

### Confidential Transfers (Tongo Cash)

Privacy-preserving transfers using ZK proofs on Starknet:

```bash
npx starkfi@latest conf-setup --key <TONGO_KEY> --contract 0x1234…         # One-time setup
npx starkfi@latest conf-balance                                           # Check state
npx starkfi@latest conf-fund 100 --token USDC                              # Fund from public
npx starkfi@latest conf-transfer 50 --recipient-x 0xABC --recipient-y 0xDEF  # Private transfer
npx starkfi@latest conf-withdraw 100                                       # Withdraw to public
npx starkfi@latest conf-ragequit                                           # Emergency exit
npx starkfi@latest conf-rollover                                           # Activate pending
```
...
View a comprehensive DeFi dashboard and optimize portfolio allocation:

```bash
npx starkfi@latest portfolio                                                          # Full dashboard
npx starkfi@latest portfolio --json                                                   # Machine-readable
npx starkfi@latest portfolio-rebalance --target "50 ETH, 30 USDC, 20 STRK" --simulate # Preview rebalance
npx starkfi@latest portfolio-rebalance --target "50 ETH, 30 USDC, 20 STRK"            # Execute rebalance
```
