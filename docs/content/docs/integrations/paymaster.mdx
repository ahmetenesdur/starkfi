...
Under the hood, transactions are sent with `feeMode: { type: "paymaster", gasToken }` to the AVNU Paymaster endpoint.
...
Under the hood, the SDK uses the v3 discriminated FeeMode union `{ type: "paymaster", gasToken: <tokenAddress> }` to inject the gas token configuration.
...
If you have an AVNU Paymaster API key for authenticated access, set it as an environment variable:

```bash
export AVNU_PAYMASTER_API_KEY=your-api-key-here
```

StarkFi automatically includes this key in all paymaster requests when set.
...
- **[Configuration](/docs/configuration)** — Gas abstraction setup guide
- **[Config Commands](/docs/cli/config)** — Full CLI config reference
- **[Security Architecture](/docs/architecture/security)** — Transaction security model
