---
title: Starkzap SDK
description: Core SDK powering wallets, tokens, staking, DCA, and transaction building
---

## Overview

[Starkzap](https://starkzap.io) is the core SDK that powers all on-chain interactions in StarkFi. It provides a unified TypeScript API for Starknet — wallet management, token operations, staking, DCA, gas abstraction, and multicall transaction building.
...
StarkFi uses TxBuilder for `batch`, `multi-swap`, `dca`, and lending operations where multiple calls must execute atomically.
