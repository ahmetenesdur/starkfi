...
3. starkfi lend-status -p Prime --collateral-token USDC  →  Verify position
```
...
3. starkfi lend-status -p Prime ...  →  Monitor Health Factor
...
| ----------------------------- | ------------------------------------------------------- |
| `-p, --pool <name>`           | Pool name or contract address                           |
| `--collateral-token <symbol>` | Token supplied to the pool (e.g., `ETH`)                |
| `--borrow-token <symbol>`     | Borrow token (optional — required to see debt position) |
| `--json`                      | Output raw JSON                                         |
