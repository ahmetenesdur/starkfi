...
4. starkfi dca-cancel <orderId>        →  Cancel when done
```
...
```bash
starkfi dca-cancel <orderId> [flags]
```
...
| Argument  | Description                 |
| --------- | --------------------------- |
| `orderId` | DCA order ID (from `dca-list`) |
...
starkfi dca-cancel abc123 --provider ekubo
```
