...
description: 36 typed error codes, retry logic, and error recovery patterns
---
...
StarkFi defines 36 typed error codes organized by domain:
...
| Code              | When                              | Resolution                                      |
| ----------------- | --------------------------------- | ----------------------------------------------- |
| `DCA_FAILED`      | DCA order operation error         | Check token pair, amount, and provider status   |

### Confidential

| Code                          | When                              | Resolution                                      |
| ----------------------------- | --------------------------------- | ----------------------------------------------- |
| `CONFIDENTIAL_FAILED`         | Confidential transfer error       | Check balance, credentials, and retry           |
| `CONFIDENTIAL_NOT_CONFIGURED` | Tongo credentials not set up      | Run `starkfi conf-setup`                        |
...
| `SDK_NOT_INITIALIZED`  | Starkzap SDK not initialized         | Ensure authentication is complete            |
| `SDK_INCOMPATIBLE`     | SDK version or mode mismatch         | Check SDK configuration and update           |
| `PROVIDER_UNAVAILABLE` | DEX provider is unavailable          | Try a different provider or retry later      |
| `ALL_PROVIDERS_FAILED` | All DEX providers failed             | Check network status and retry               |
...
Most CLI commands use the `withAuthenticatedWallet` runner from `lib/command-runner.ts`, which centralizes session auth, SDK initialization, deployment checks, spinner lifecycle, and error handling:

```typescript
.action(async (amount, token, opts) => {
	await withAuthenticatedWallet("Processing...", async (ctx) => {
		// ctx provides: wallet, session, sdk, chainId, network, spinner
		const tokenObj = resolveToken(token, ctx.chainId);
		// ... business logic
		ctx.spinner.succeed("Done");
	}, { ensureDeployed: true, onError: "Operation failed" });
});
```

The runner automatically catches errors, stops the spinner, calls `formatError()`, and exits with code 1. Errors inside the callback should be thrown as `StarkfiError` — the runner handles the rest.

For commands that don't need a wallet (e.g., `tx-status`, `deploy`), the traditional pattern is used:
...
- **[Security Architecture](/docs/architecture/security)** — Transaction security model and error masking
- **[Architecture Overview](/docs/architecture)** — System design and design patterns
