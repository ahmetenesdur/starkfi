...
Network switching is **instant** — no re-login required. The config setting overrides the network that was active when you authenticated.

`config list` shows the effective network with its source:

```
network    sepolia (config override, session: mainnet)
feeMode    gasless (pays STRK via Paymaster)
```

> **How it works:** StarkFi resolves the network at runtime using a priority chain: `config.network` (user override) → `session.network` (login default). This means you can authenticate once on mainnet and freely switch to sepolia and back without re-authenticating.
...
This removes custom RPC URL, network override, gas token, and gasfree mode settings in one command. After reset, the network reverts to whatever was set during authentication.
